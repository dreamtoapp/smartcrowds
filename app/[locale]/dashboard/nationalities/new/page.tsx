...
import { createNationality } from '@/app/actions/nationalities/actions';
...
export default function NewNationalityPage() {
  ...
  const onSubmit = async (values: z.infer<typeof schema>) => {
    startTransition(async () => {
      const result = await createNationality(values);
      if (result.success) {
        router.push(`/${locale}/dashboard/nationalities`);
      } else {
        showErrorSwal(result.error || (locale === 'ar' ? 'فشل إنشاء الجنسية' : 'Failed to create nationality'), locale);
      }
    });
  };
  ...
  return (
    <section className="space-y-4 max-w-xl">
      <h1 className="text-2xl font-bold">{locale === 'ar' ? 'إنشاء جنسية' : 'Create Nationality'}</h1>
